...
namespace OpenUO.Core.Patterns
{
	...
	public class SafeDictionary<TKey, TValue> : IDisposable
	{
		...
		public void Dispose()
		{
			lock (_Padlock)
			{
				var disposableItems = from item in _Dictionary.Values where item is IDisposable select item as IDisposable;
	...
	public class IoCRegistrationException : Exception
	{
		...
		public IoCRegistrationException(Type registerType, Type implementationType, Exception innerException)
			: base(
				String.Format(GENERIC_CONSTRAINT_ERROR_TEXT, registerType.FullName, implementationType.FullName), innerException)
		{ }
	}
	...
	public class IoCAutoRegistrationException : Exception
	{
		...
		{
			var typeNames = from type in types select type.FullName;
	...
	public sealed class NamedParameterOverloads : Dictionary<string, object>
	{
		...
		public static NamedParameterOverloads Default { get { return _Default; } }
	...
	public sealed class ResolveOptions
	{
		...
		public UnregisteredResolutionActions UnregisteredResolutionAction { get { return _UnregisteredResolutionAction; } set { _UnregisteredResolutionAction = value; } }

		public NamedResolutionFailureActions NamedResolutionFailureAction { get { return _NamedResolutionFailureAction; } set { _NamedResolutionFailureAction = value; } }
		...
		/// </summary>
		public static ResolveOptions FailUnregisteredOnly { get { return _FailUnregisteredOnly; } }
	}
	#endregion
	...
	public sealed class Container : IContainer
	{
		...
		public void AutoRegister(Func<Type, bool> registrationPredicate)
		{
			...
			AutoRegisterInternal(
				AppDomain.CurrentDomain.GetAssemblies().Where(a => !IsIgnoredAssembly(a)), true, registrationPredicate);
#else
...
#if APPDOMAIN_GETASSEMBLIES
			AutoRegisterInternal(
				AppDomain.CurrentDomain.GetAssemblies().Where(a => !IsIgnoredAssembly(a)), ignoreDuplicateImplementations, null);
#else
		...
		public void AutoRegister(bool ignoreDuplicateImplementations, Func<Type, bool> registrationPredicate)
		{
			...
			AutoRegisterInternal(
				...
				ignoreDuplicateImplementations,
		...
		public RegisterOptions Register(Type registerType, Type registerImplementation, object instance)
		{
			return RegisterInternal(
				registerType, string.Empty, new InstanceFactory(registerType, registerImplementation, instance));
		}
		...
		/// <returns>RegisterOptions for fluent API</returns>
		public RegisterOptions Register<RegisterType, RegisterImplementation>() where RegisterType : class
		...
		/// <param name="name">Name of registration</param>
		/// <returns>RegisterOptions for fluent API</returns>
		public RegisterOptions Register<RegisterType, RegisterImplementation>(string name) where RegisterType : class
			where RegisterImplementation : class, RegisterType
		{
		...
		public RegisterOptions Register<RegisterType>(
			Func<Container, NamedParameterOverloads, RegisterType> factory, string name) where RegisterType : class
		{
			if (factory == null)
			{
				throw new ArgumentNullException("factory");
		...
		public object Resolve(Type resolveType, string name)
		{
			return ResolveInternal(
				new TypeRegistration(resolveType, name), NamedParameterOverloads.Default, ResolveOptions.Default);
		}
		...
		public bool TryResolve<ResolveType>(
			NamedParameterOverloads parameters, ResolveOptions options, out ResolveType resolvedType) where ResolveType : class
		{
			try
			{
				resolvedType = Resolve<ResolveType>(parameters, options);
		...
		private abstract class ObjectFactoryBase
		{
			...
			public virtual ObjectFactoryBase SingletonVariant { get { throw new IoCRegistrationException(GetType(), "singleton"); } }

			public virtual ObjectFactoryBase MultiInstanceVariant { get { throw new IoCRegistrationException(GetType(), "multi-instance"); } }

			public virtual ObjectFactoryBase StrongReferenceVariant { get { throw new IoCRegistrationException(GetType(), "strong reference"); } }

			public virtual ObjectFactoryBase WeakReferenceVariant { get { throw new IoCRegistrationException(GetType(), "weak reference"); } }
			...
			public abstract object GetObject(
				Type requestedType, Container container, NamedParameterOverloads parameters, ResolveOptions options);
		...
		private class MultiInstanceFactory : ObjectFactoryBase
		{
			...
			public override ObjectFactoryBase SingletonVariant { get { return new SingletonFactory(registerType, registerImplementation); } }

			public override ObjectFactoryBase MultiInstanceVariant { get { return this; } }
		...
		private class DelegateFactory : ObjectFactoryBase
		{
			...
			public override ObjectFactoryBase WeakReferenceVariant { get { return new WeakDelegateFactory(registerType, _factory); } }

			public override ObjectFactoryBase StrongReferenceVariant { get { return this; } }
		...
		private class WeakDelegateFactory : ObjectFactoryBase
		{
			...
			public override ObjectFactoryBase WeakReferenceVariant { get { return this; } }
		...
		private class InstanceFactory : ObjectFactoryBase, IDisposable
		{
			...
			public override ObjectFactoryBase MultiInstanceVariant { get { return new MultiInstanceFactory(registerType, registerImplementation); } }

			public override ObjectFactoryBase WeakReferenceVariant { get { return new WeakInstanceFactory(registerType, registerImplementation, _instance); } }
		...
		private class SingletonFactory : ObjectFactoryBase, IDisposable
		{
			...
			public override ObjectFactoryBase SingletonVariant { get { return this; } }
		...
		/// </summary>
		public static Container Current { get { return _Current; } }
		#endregion
			...
			IEnumerable<Assembly> assemblies, bool ignoreDuplicateImplementations, Func<Type, bool> registrationPredicate)
		{
			...
			{
				var types =
					assemblies.SelectMany(a => a.SafeGetTypes()).Where(t => !IsIgnoredType(t, registrationPredicate)).ToList();
...
#endif
				t =>
				(t.GetConstructors(BindingFlags.Instance | BindingFlags.Public).Length == 0) && !(t.IsInterface || t.IsAbstract),
			};
	...
	// Only register the Messenger singleton if we are the root container
			...
			TypeRegistration registration, NamedParameterOverloads parameters, ResolveOptions options)
		{
			...
			if (registration.Type.IsGenericType)
			{
				TypeRegistration openTypeRegistration = new TypeRegistration(
					registration.Type.GetGenericTypeDefinition(), registration.Name);
		...
		private object GetLazyAutomaticFactoryRequest(Type type)
		{
				 ...
				 type.GetGenericArguments()[1] == typeof(IDictionary<string, object>)))
			{
				...
				MethodInfo resolveMethod = typeof(Container).GetMethod(
					"Resolve", new[] {typeof(String), typeof(NamedParameterOverloads)});
				resolveMethod = resolveMethod.MakeGenericMethod(returnType);
			...
			for (int parameterIndex = 0; parameterIndex < ctorParams.Count(); parameterIndex++)
			{
				...
				try
				{
					args[parameterIndex] = parameters.ContainsKey(currentParam.Name)
											   ...
											   : ResolveInternal(
												   new TypeRegistration(currentParam.ParameterType), NamedParameterOverloads.Default, options);
				}
				catch (IoCResolutionException ex)
				{
					...
					// be resolved.
					throw new IoCResolutionException(typeToConstruct, ex);
				}
				catch (Exception ex)
				{
					throw new IoCResolutionException(typeToConstruct, ex);
		...
		private void BuildUpInternal(object input, ResolveOptions resolveOptions)
		{
			var properties = from property in input.GetType().GetProperties()
							 where
								 (property.GetGetMethod() != null) && (property.GetSetMethod() != null) && !property.PropertyType.IsValueType
							 select property;
			...
			foreach (PropertyInfo property in properties)
			{
				if (property.GetValue(input, null) == null)
				{
					try
					{
						property.SetValue(
							input,
							ResolveInternal(new TypeRegistration(property.PropertyType), NamedParameterOverloads.Default, resolveOptions),
							null);
					}
					catch (IoCResolutionException)
					{
		...
		private IEnumerable<object> ResolveAllInternal(Type resolveType, bool includeUnnamed)
		{
			...
			return
				registrations.Select(
					registration => ResolveInternal(registration, NamedParameterOverloads.Default, ResolveOptions.Default));
		}
